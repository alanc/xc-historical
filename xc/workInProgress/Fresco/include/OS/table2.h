...
inline Boolean key_equal(long k1, long k2) { return k1 == k2; }
inline Boolean key_equal(const void* k1, const void* k2) { return k1 == k2; }
#endif
...
Boolean Table2::find(Value& v, Key1 k1, Key2 k2) { \
	...
	if (key_equal(e->key1_, k1) && key_equal(e->key2_, k2)) { \
	    v = e->value_; \
	    return true; \
	} \
    } \
...
void Table2::remove(Key1 k1, Key2 k2) { \
	...
	if (key_equal(e->key1_, k1) && key_equal(e->key2_, k2)) { \
	    *a = e->chain_; \
		...
		!(key_equal(e->key1_, k1) && key_equal(e->key2_, k2)) \
	    ); \
	    if (e != nil) { \
