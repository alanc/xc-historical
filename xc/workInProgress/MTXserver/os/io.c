    ...
    register int needed;
{
    ...
    if (!oci)
    {
	ErrorF("ReadRequestFromClient: No Input buffer\n");
	return (-1);
    }
...
/*
 * XXX:SM - This is a really a call to "pthread_setintr", which isn't 
 * referenced in the OSF DCE Application Development Reference.  I'm
 * going to assume that the thread safe version of libc.a will contain a 
 * version of read that works.
 * It looks like this code is going to enable interrupts while the read is
 * taking place???
 */
#ifndef AIXV3
	X_SETINTR(PTHREAD_INTR_ENABLE);
#endif
        result = read(oc->fd, oci->buffer + oci->bufcnt, 
		      oci->size - oci->bufcnt);

#ifndef AIXV3
	X_SETINTR(PTHREAD_INTR_DISABLE);
#endif
 ...
 *
 * reset the input buffer and just read ConnectionSetup and 
 * Authorization data.
 *
 ********************************************************************/
 
Bool
ReadInitialConnection(oc, swapped)
    OsCommPtr oc;
    int *swapped;
{
    register ConnectionInputPtr oci;
    ...
    result = read(oc->fd, oci->buffer, oci->size - oci->bufcnt);
    if (result <= 0)
    {
	return (FALSE);
    }
    ...
    {
	*swapped = SWAP;
	(void)SwapConnClientPrefix(prefix);
    }	
    else
    {
	*swapped = NOSWAP;
    }
    lengthAuth = ((prefix->nbytesAuthProto + 3) & ~3) +
		 ((prefix->nbytesAuthString + 3) & ~3);
    if ((needed = lengthAuth + sz_xConnClientPrefix) > oci->size)
    {
	char *ibuf;
	...
	result = read(oc->fd, oci->buffer + oci->size, needed - oci->size);
	if (result <= 0)
	{
	    return (FALSE);
	}
