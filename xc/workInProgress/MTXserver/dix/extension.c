...
ProcQueryExtension(client)
    ClientPtr client;
{
    REPLY_DECL(xQueryExtensionReply,reply);
    int i, j;
    ...
    MTX_REP_LOCK_PENDING_OPERATION_QUEUE(client, CM_XQueryExtension);
...
ProcListExtensions(client)
    ClientPtr client;
{
    REPLY_DECL(xListExtensionsReply,reply);
    char *bufptr, *buffer;
    int total_length = 0;
    ...
    MTX_REP_LOCK_PENDING_OPERATION_QUEUE(client, CM_XListExtensions);
	...
	buffer = bufptr = (char *)MTX_LOCAL_ALLOC(total_length);
	if (!buffer)
	{
