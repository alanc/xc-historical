...
/* $XConsortium: window.c,v 5.103 93/12/02 12:56:12 rob Exp $ */
...
void
CalculateConfigureRegion(client, pWin, pRegion)
    ClientPtr client;
    WindowPtr pWin;
    RegionPtr pRegion;
{
    register DrawablePtr pDraw;
    register int rx, ry, rw, rh;
    register int bw;
    register Mask mask;
    register XID *pVlist;
    register int result;
    ...
    /*-----------------------------------------------------------
     * Union the geometry info in the protocol request with
     * the current window geometry to produce an acurate region
     * to lock. 
     *-----------------------------------------------------------*/
    ...
    /*------------------------------------------------
     * Get the x, y, width, height, and border width
     * from the protocol request.
     *-----------------------------------------------*/
    ...
    if (CWBorderWidth & mask)
    {
        bw = max (pWin->borderWidth, (unsigned int)*pVlist);
        pVlist++;
    }
    else
    {
    ...
    /*--------------------------
     * Build the region to lock.
     *--------------------------*/

    pRegion->extents.x1 = max(0,(min(pDraw->x, rx) - bw));
    pRegion->extents.y1 = max(0,(min(pDraw->y, ry) - bw));
    pRegion->extents.x2 = max((pDraw->x + pDraw->width),(rx + rw)) + bw;
