/* $XConsortium: imake.c,v 1.78 93/08/14 17:37:32 rws Exp $ */
...
makeit()
{
	 ...
	 */
#ifdef WIN32
	status = _spawnvp(_P_WAIT, make, make_argv);
	if (status < 0)
		LogFatal("Cannot spawn %s.", make);
	if (status > 0)
		LogFatalI("Exit code %d.", status);
#else
	pid = fork();
	...
	register char	*line;
{
	...
	if (*line == '#') {
		    ...
		    pend[3] == 'R')
		{
			...
			pend[7] >= '0' && pend[7] <= '9')
		    {
			sprintf(varbuf, "%0.3d", xvariable);
			strncpy(pend+4, varbuf, 3);
			xvariables[pend[7] - '0'] = xvariable;
			xvariable = (xvariable + 1) % 1000;
		    }
		    else if (pend[4] == 'u' && pend[5] == 's' &&
			     pend[6] == 'e' && pend[7] >= '0' &&
			     pend[7] <= '9')
		    {
			sprintf(varbuf, "%0.3d", xvariables[pend[7] - '0']);
			strncpy(pend+4, varbuf, 3);
		    }
		}
#endif
