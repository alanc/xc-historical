...
Top:      Red 2D text showing various kinds of alignment: \n\
          Normal, Center-Half, Right-Bottom, Left-Top, Right-Base, Left-Cap.\n\
          Markers are drawn on the text point for each text primitive.\n\
Center:   1. Red 2D text rendered using bundled attributes.\n\
             (color and spacing)\n\
          2. Text drawn in blue\n\
          3. 2D text rendered with 2 different up vectors: \n\
             (0,-1),(1,2)\n\
Mid-Left: 2D text drawn in 4 different paths: \n\
          Top, Down, Right, Left\n\
Bottom:   1. 2D text drawn with char height: -0.02, 0, 0.04.\n\
          2. 2D text drawn with char expansion: -0.4, 0, 1.2.\n\
          3. 2D text drawn with char spacing: -0.5, 0, 1.0.\n\
";
...
/* char height */
    for (i=0; i < sizeof(height)/sizeof(double); i++) {
    	...
    	TEXT2D(&pt1, sizeof(str_height[i]) - 1, str_height[i])
    	pt1.y += 0.08;
    }
...
/* char expansion */
    ...
    for (i=0; i < sizeof(exp)/sizeof(double); i++) {
    	...
    	TEXT2D(&pt1, sizeof(str_exp[i]) - 1, str_exp[i])
    	pt1.y += 0.08;
    }
...
/* char spacing */
    ...
    for (i=0; i < sizeof(space)/sizeof(double); i++) {
    	...
    	TEXT2D(&pt1, sizeof(str_space[i]) - 1, str_space[i])
    	pt1.y += 0.08;
    }
...
/* text path */
    ...
    for (i=0; i < sizeof(path)/sizeof(int); i++) {
    	...
    	TEXT2D(&pt1, sizeof(STR_PEX) - 1, STR_PEX)
    }
...
/* up vector */
    pt1.x = 0.375;
    pt1.y = 0.3;
    SET_CHAR_UP_VECTOR(upvec1)
    TEXT2D(&pt1, sizeof(str_upvec[0]) - 1, str_upvec[0])

    pt1.x = 0.55;
    pt1.y = 0.4;
    SET_CHAR_UP_VECTOR(upvec2)
    TEXT2D(&pt1, sizeof(str_upvec[1]) - 1, str_upvec[1])

    SET_CHAR_UP_VECTOR(upvec3)
...
/* alignment and color index */
    ...
    for (i=0,k=0; i < 2; i++) {
    	for (j=0; j < 3; j++,k++) {
    	    ...
    	    TEXT2D(&pt1, sizeof(str_align[k]) - 1, str_align[k])
	    MARKER2D(1, &pt1)
	    pt1.y += 0.1;
	}
	pt1.x += 0.4;
...
/* 3D text and text color */
    ...
    TEXT(&pt2, &vec1, &vec2, sizeof(STR_3DTEXT) - 1, STR_3DTEXT)
...
/* text bundle */
    ...
    pt1.x = 0.4;
    pt1.y = 0.6;
    TEXT2D(&pt1, sizeof(STR_BUNDLE) - 1, STR_BUNDLE)
...
testproc(dpy,window,x,y,w,h)
    ...
    int			x,y,w,h;
{
    image_testproc("textAttrs", "text attributes", test_description,
	dpy,window,x,y,w,h);
} 
