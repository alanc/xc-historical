...
Change Structure Reference Tests:\n\n\
You should see 2 white polylines.\n\
";
    ...
    Display             *dpy;
{
    ...
    PEXExecuteStructure(dpy, object, PEXOCStore, nothing);
}
    ...
    PEXOCRequestType    req_type;
{
    ...
    /* change structure references from null to line
       so now expect to see two lines
     */
    PEXChangeStructureRefs(dpy, nothing, line);
}
...
testproc(dpy,window,x,y,w,h)
    ...
    int                 x,y,w,h;
{
    image_testproc("struct_ref", "structure reference", test_description,
        dpy,window,x,y,w,h);
}
